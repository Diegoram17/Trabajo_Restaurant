...
| 2 | Núcleo de exactitud | Importes enteros, redondeo único medio-arriba, reparto con residuo, `dia_operativo()`, resolución de vigencias. **Puerta de entrada**: acá se decide la **capa de acceso a datos** para las consultas de dominio —`pg` crudo, query builder u ORM— con su propio ADR. Se difirió del #1 porque ahí no hacía falta, pero *resolución de vigencias* es la primera consulta de dominio del proyecto. **Hereda del #1 una obligación**: el #1 crea `vigente_desde` sin restricción temporal —no hay filas ni camino de escritura todavía—, así que el rechazo de fechas pasadas contra el día operativo (ADR-0022, ADR-0028) **se implementa acá**. Sin esto, la columna queda sin su regla y nadie se entera | #1 | — |
...
| 24 | Gestión de personal y horarios | ABM de personas y roles, regeneración de PIN, desactivación, `HorarioProgramado`. **Hereda del #3 una obligación**: `sueldo_fijo` no está en el `persona` que crea #3 (propuesta #3, Q7); la columna llega acá, con su primer writer | #5 | — |
| 25 | Calendarios, costos y parámetros | `CalendarioApertura`, `ConfiguracionCostos` versionada, **ambas con `creada_por` y `creada_en` visibles** (SEC-08), rechazo de fechas pasadas, rotación del PIN de cocina. **Hereda del #1 una obligación**: el #1 crea las columnas de porcentaje sin restricción de rango porque no tiene camino de escritura; el rechazo de valores fuera de `0..10000` puntos básicos (ADR-0039) se implementa acá, donde nace la primera escritura, y conviene que viva también como `CHECK` en la base y no solo en la pantalla. **Y toda escritura de dinero o porcentaje va por parámetro enlazado**: se verificó que un literal SQL fraccionario contra una columna `integer` no falla, se redondea solo | #2, #11 | — |
